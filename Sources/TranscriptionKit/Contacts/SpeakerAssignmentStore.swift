...
@MainActor
public enum SpeakerAssignmentStore {
    /// Bind (or rebind) `slot` in `session` to a contact — one assignment per slot (upsert).
    @discardableResult
    public static func assign(slot: Int, contactIdentifier: String, displayName: String,
    ...
    public static func clear(slot: Int, from session: TranscriptSession, in context: ModelContext) {
    ...
    public static func nameBySlot(for session: TranscriptSession) -> [Int: String] {
...
public enum SessionPeople {
    public static func names(for session: TranscriptSession) -> [String] {
        var seen = Set<String>()
        var result: [String] = []
