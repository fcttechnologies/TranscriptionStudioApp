...
public struct CalendarWriteAction: ConfirmableWrite {
    public typealias Output = Void

    private let initialDraft: CalendarDraft

    public init(draft: CalendarDraft) {
        self.initialDraft = draft
    }

    public func makeDraft() -> CalendarDraft { initialDraft }

    public func commit(_ draft: CalendarDraft) async throws {
...
public struct ReminderWriteAction: ConfirmableWrite {
    public typealias Output = Void

    private let initialDraft: ReminderDraft

    public init(draft: ReminderDraft) {
        self.initialDraft = draft
    }

    public func makeDraft() -> ReminderDraft { initialDraft }

    public func commit(_ draft: ReminderDraft) async throws {
        let store = EKEventStore()
        let granted: Bool
        do {
