...
enum TranscriptionSyncSchema {
    ...
    static let postgresSchema = "transcriptionstudio"
    ...
    static let version = "transcriptionstudio.1"
...
extension TranscriptSession: SyncedModel {
    static var syncTableName: String { "transcriptionstudio.transcript_session" }
    static var syncIDKeyPath: KeyPath<TranscriptSession, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<TranscriptSession> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<TranscriptSession> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<TranscriptSession> { FetchDescriptor() }
...
extension StoredSegment: SyncedModel, TranscriptSessionChild {
    static var syncTableName: String { "transcriptionstudio.stored_segment" }
    static var syncIDKeyPath: KeyPath<StoredSegment, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<StoredSegment> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<StoredSegment> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<StoredSegment> { FetchDescriptor() }
...
extension TranscriptDecision: SyncedModel, TranscriptSessionChild {
    static var syncTableName: String { "transcriptionstudio.transcript_decision" }
    static var syncIDKeyPath: KeyPath<TranscriptDecision, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<TranscriptDecision> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<TranscriptDecision> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<TranscriptDecision> { FetchDescriptor() }
...
extension TranscriptActionItem: SyncedModel, TranscriptSessionChild {
    static var syncTableName: String { "transcriptionstudio.transcript_action_item" }
    static var syncIDKeyPath: KeyPath<TranscriptActionItem, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<TranscriptActionItem> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<TranscriptActionItem> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<TranscriptActionItem> { FetchDescriptor() }
...
extension TranscriptEvent: SyncedModel, TranscriptSessionChild {
    static var syncTableName: String { "transcriptionstudio.transcript_event" }
    static var syncIDKeyPath: KeyPath<TranscriptEvent, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<TranscriptEvent> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<TranscriptEvent> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<TranscriptEvent> { FetchDescriptor() }
...
extension TranscriptPerson: SyncedModel, TranscriptSessionChild {
    static var syncTableName: String { "transcriptionstudio.transcript_person" }
    static var syncIDKeyPath: KeyPath<TranscriptPerson, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<TranscriptPerson> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<TranscriptPerson> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<TranscriptPerson> { FetchDescriptor() }
...
extension TranscriptPlace: SyncedModel, TranscriptSessionChild {
    static var syncTableName: String { "transcriptionstudio.transcript_place" }
    static var syncIDKeyPath: KeyPath<TranscriptPlace, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<TranscriptPlace> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<TranscriptPlace> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<TranscriptPlace> { FetchDescriptor() }
...
extension SpeakerAssignment: SyncedModel, TranscriptSessionChild {
    static var syncTableName: String { "transcriptionstudio.speaker_assignment" }
    static var syncIDKeyPath: KeyPath<SpeakerAssignment, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<SpeakerAssignment> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<SpeakerAssignment> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<SpeakerAssignment> { FetchDescriptor() }
...
extension MacPresence: SyncedModel {
    static var syncTableName: String { "transcriptionstudio.mac_presence" }
    static var syncIDKeyPath: KeyPath<MacPresence, UUID> { \.id }

    static func descriptor(forSyncIDs ids: [UUID]) -> FetchDescriptor<MacPresence> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.id) })
    }

    static func descriptor(forPersistentIDs ids: [PersistentIdentifier]) -> FetchDescriptor<MacPresence> {
        FetchDescriptor(predicate: #Predicate { ids.contains($0.persistentModelID) })
    }

    static var allRecordsDescriptor: FetchDescriptor<MacPresence> { FetchDescriptor() }
