...
struct CloningVoiceProfile: Sendable {
    ...
    let primaryVoice: String

    init(references: [String: Reference], primaryVoice: String) {
        self.references = references
        self.primaryVoice = primaryVoice
    }

    /// The voice ids, sorted for stable help text and error copy.
    var voiceIDs: [String] { references.keys.sorted() }

    /// Load a profile file. `ref_audio` paths resolve relative to the profile's own folder.
    static func load(from url: URL) throws -> CloningVoiceProfile {
...
enum PromptTranscript {
    ...
    static let promptWindowSeconds: TimeInterval = 5.0
    ...
    static let cutGuardSeconds: TimeInterval = 0.05
    ...
    /// one word up.
    static let tailPadSeconds: TimeInterval = 0.08
    ...
    static let minSentenceCutSeconds: TimeInterval = 3.0
    ...
    struct MatchedPrompt: Sendable, Equatable {
        let text: String
        let clipSeconds: TimeInterval

        init(text: String, clipSeconds: TimeInterval) {
            self.text = text
    ...
    static func matched(words: [AsrWord],
...
enum SentenceChunker {
    ...
    static func halves(of chunk: String) -> (String, String)? {
