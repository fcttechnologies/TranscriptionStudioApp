...
enum SortformerModelError: Error, LocalizedError {
...
struct SortformerManifestEntry: Codable, Sendable {
    let name: String     // path relative to the Models root
    let bytes: Int
    let sha256: String?
    init(name: String, bytes: Int, sha256: String? = nil) {
        self.name = name; self.bytes = bytes; self.sha256 = sha256
    }
}

/// The integrity manifest for a provisioned model set.
struct SortformerManifest: Codable, Sendable {
    let files: [SortformerManifestEntry]
    init(files: [SortformerManifestEntry]) { self.files = files }

    static let filename = "sortformer_manifest.json"

    /// The HF-original sizes (used when no local manifest is present).
    static let hfDefault = SortformerManifest(files: [
...
struct SortformerDownloadProgress: Sendable {
    let file: String
    let fraction: Double?   // 0...1 for the current file, nil when indeterminate
    let completedFiles: Int
    let totalFiles: Int
}

struct SortformerModelStore: Sendable {
    /// The HF-original expected sizes, exposed for scripts/tests (see also SortformerManifest.hfDefault).
    static let mainMlirbBytes = 236_655_368
    static let melFilterBytes = 128 * 257 * 4   // 131584
    ...
    var modelURL: URL { root.appendingPathComponent("sortformer_float16.aimodel", isDirectory: true) }
    var mainMlirbURL: URL { modelURL.appendingPathComponent("main.mlirb") }
    var melFiltersURL: URL { root.appendingPathComponent("sortformer_mel_filters_128x257.f32") }
    var metadataURL: URL { root.appendingPathComponent("metadata.json") }
    var manifestURL: URL { root.appendingPathComponent(SortformerManifest.filename) }

    /// The active manifest: the local one if written next to the artifacts, else the HF default.
    func effectiveManifest() -> SortformerManifest {
    ...
    func loadMelFilters() throws -> [Float] {
    ...
    func writeManifest(_ manifest: SortformerManifest) throws {
