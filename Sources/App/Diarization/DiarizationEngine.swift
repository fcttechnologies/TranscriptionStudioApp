...
struct SpeakerTurn: Sendable, Codable, Hashable, Identifiable {
    var id: String { "\(speakerIndex)-\(start)-\(end)-\(isCommitted)" }
...
struct DiarizationResult: Sendable {
    ...
    let frames: SpeakerFrameMatrix

    init(turns: [SpeakerTurn], frames: SpeakerFrameMatrix) {
        self.turns = turns
        self.frames = frames
    }
}
...
struct DiarizationUpdate: Sendable {
    let turns: [SpeakerTurn]
    let frames: SpeakerFrameMatrix
...
protocol DiarizationEngine: AnyObject, Sendable {
