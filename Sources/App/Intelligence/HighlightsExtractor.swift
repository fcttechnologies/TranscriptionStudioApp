...
struct HighlightsExtractor: Sendable {
    private let extractor: GuidedExtractor

    init(extractor: GuidedExtractor = GuidedExtractor()) {
    ...
    func extract(from transcript: String, referenceDate: Date) async throws -> SessionHighlights {
    ...
    @MainActor
    func schedule(for session: TranscriptSession, modelContext: ModelContext, force: Bool = false) {
    ...
    /// in-memory container.
    @MainActor
    static func apply(_ highlights: SessionHighlights, to session: TranscriptSession,
