...
struct EcosystemActionItemRef: Sendable, Equatable {
    let id: UUID
    ...
    init(id: UUID, label: String, detail: String) {
        self.id = id
        self.label = label
        self.detail = detail
    }
...
enum EcosystemActionStore {
    ...
    static func actionItems(forSessionID id: UUID,
                                   in container: ModelContainer = AppModelContainer.shared) -> [EcosystemActionItemRef] {
        guard let session = session(id, in: container) else { return [] }
