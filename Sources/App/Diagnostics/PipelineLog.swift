...
enum PipelineStage: String, Sendable, Codable, CaseIterable {
...
enum PipelineEventLevel: String, Sendable, Codable {
...
struct PipelineEvent: Sendable, Identifiable {
    let id: UUID
    let date: Date
    let sessionID: UUID?
...
final class PipelineRecorder: Sendable {
    ...
    init(store: InspectorStore?, stateReporter: (any PipelineStateReporting)? = nil) {
        self.store = store
    ...
    func time<T>(_ stage: PipelineStage,
                        sessionID: UUID? = nil,
                        _ message: String,
                        metadata: [String: String] = [:],
