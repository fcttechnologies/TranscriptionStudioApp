...
struct SystemLoadSample: Sendable, Identifiable {
    let id: UUID
    let date: Date
    ...
    init(thermalState: ProcessInfo.ThermalState, cpuPercent: Double, memoryFootprint: UInt64) {
        self.id = UUID()
        self.date = Date()
...
final class SystemLoadSampler: Sendable {
    private let store: InspectorStore
    private let interval: TimeInterval
    private let task = Mutex<Task<Void, Never>?>(nil)

    init(store: InspectorStore, interval: TimeInterval = 1.0) {
        self.store = store
    ...
    private static func cpuTicks() -> Double {
        ...
        // Safety: Mach's `task_info` fills a caller-owned struct through an `integer_t` view of
        // it; `count` is that struct's size in words, so the write is bounded by `info`.
        let kr = withUnsafeMutablePointer(to: &info) {
            unsafe $0.withMemoryRebound(to: integer_t.self, capacity: Int(count)) {
                unsafe task_info(mach_task_self_, task_flavor_t(TASK_THREAD_TIMES_INFO), $0, &count)
            }
        }
    ...
    private static func memoryFootprint() -> UInt64 {
        ...
        // Safety: as above, the kernel writes at most `count` words into the caller-owned struct.
        let kr = withUnsafeMutablePointer(to: &info) {
            unsafe $0.withMemoryRebound(to: integer_t.self, capacity: Int(count)) {
