...
final class ReadAloudController {
    ...
    struct NowSpeaking: Equatable, Sendable {
        let sessionID: UUID
        let title: String
    }
    ...
    private(set) var nowSpeaking: NowSpeaking?
    ...
    init(makeEngine: @escaping @Sendable () -> any TtsEngine = { TTSKitTtsEngine() }) {
    ...
    func speak(session: TranscriptSession) {
    ...
    func speak(sessionID: UUID, title: String, text: String) {
    ...
    private func schedule(chunk: SynthesizedSpeechChunk, wire: SpeakWire) {
        ...
        // count just requested, and the source pointer is valid for the closure.
        if let channel = unsafe buffer.floatChannelData?[0] {
            chunk.samples.withUnsafeBufferPointer { source in
                unsafe channel.update(from: source.baseAddress!, count: chunk.samples.count)
            }
        }
