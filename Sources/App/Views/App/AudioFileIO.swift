...
enum AudioFileIO {
    ...
    /// it back into memory.
    static func encodeAAC(samples: [Float]) throws -> Data {
    ...
    static func decodeAAC(_ data: Data) throws -> [Float] {
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString)
            .appendingPathExtension("m4a")
        defer { try? FileManager.default.removeItem(at: tempURL) }
    ...
    static func synthesize(turns: [(start: TimeInterval, end: TimeInterval, slot: Int)],
