...
final class RecordingController {
    ...
    struct RecordingError: Identifiable, Sendable, Equatable {
        let id = UUID()
        let message: String
    ...
    private(set) var mode: Mode = .room
    ...
    private(set) var segments: [AttributedSegment] = []
    ...
    private(set) var liveTurns: [SpeakerTurn] = []
    ...
    private(set) var diarizationUnavailable = false
    ...
    var archivedSamples: [Float] { archiver.archive }
    ...
    struct CaptureInput {
        ...
        init(source: any CaptureSource, tracks: [AudioTrack]) {
            self.source = source
            self.tracks = tracks
        }
    }
    ...
    typealias CaptureFactory = @MainActor (Mode, UUID, PipelineRecorder) -> [CaptureInput]

    /// The default factory: deterministic mock sources (previews, tests, engine-less demos).
    static let mockCaptureFactory: CaptureFactory = { mode, _, _ in
        switch mode {
        case .room:
