...
/// Test bundles get no generated resource accessors, so the bundle's resources are reached
/// through this marker class's own bundle.
private final class TestResources { static let bundle = Bundle(for: TestResources.self) }

@Suite("Sortformer mel frontend")
struct SortformerMelTests {
    private static func loadGolden() throws -> MelGolden {
        let url = try #require(TestResources.bundle.url(forResource: "mel_golden", withExtension: "json"),
